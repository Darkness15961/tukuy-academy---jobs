-- Incluye imagenPosicion (object-position) desde documento_borrador_curso.
begin;
...
begin
  ...
  select nullif(trim(d.documento->>'imagenPosicion'), '')
    into v_posicion
  from public.documento_borrador_curso d
  ...
  return jsonb_build_object(
    ...
    'curso', jsonb_build_object(
      ...
      'imagenPosicion', coalesce(v_posicion, '50% 50%'),
