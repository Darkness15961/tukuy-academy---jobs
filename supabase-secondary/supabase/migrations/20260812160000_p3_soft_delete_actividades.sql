-- P3 LMS: soft-delete de módulos/actividades (conserva progreso).
-- Ejecutar en secundaria después de 12150000.
  ...
  if v_marcar then
    if exists (
      ...
      where mo_prev.version_curso_id = mo_cur.version_curso_id
        and coalesce(mo_prev.activo, true)
        and coalesce(prev.activa, true)
        and (
...
grant execute on function public.servicio_completar_actividad(uuid, uuid, uuid, numeric, boolean)
  to service_role;
grant execute on function public._servicio_recalcular_progreso_y_cert(uuid, uuid)
  to service_role;
