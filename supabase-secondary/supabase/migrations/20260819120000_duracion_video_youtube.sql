-- Persiste duracionMinutos enviada desde el constructor (YouTube Data API).
-- Ejecutar en SQL Editor del proyecto SECUNDARIO.
begin;

update public.actividad_curso
set instrucciones = null
where instrucciones ~* '^\s*(Video:\s*)?https?://';
...
immutable
as $$
  -- Nota: no usar greatest(1, NULL) — en PostgreSQL eso devuelve 1 y anula el fallback.
  select coalesce(
    (
      select minutos
      from (
...
begin
      ...
      loop
        ...
        then
          ...
          update public.actividad_curso set
            ...
            duracion_minutos = public._servicio_duracion_minutos_item(v_item, v_tipo_act),
            orden = v_orden_act,
          ...
          insert into public.actividad_curso (
            ...
            public._servicio_duracion_minutos_item(v_item, v_tipo_act),
            v_orden_act,
            v_banco,
            v_url,
            true
        ...
        v_items_out := v_items_out || jsonb_build_array(
          jsonb_build_object(
            ...
            'duracionMinutos', public._servicio_duracion_minutos_item(v_item, v_tipo_act)
          )
        );
      end loop;
    else
  ...
  update public.version_curso set
    horas = greatest(1, ceil((
      select coalesce(sum(greatest(a.duracion_minutos, 1)), 0)::numeric / 60
      from public.actividad_curso a
      join public.modulo_curso m on m.id = a.modulo_curso_id
      where m.version_curso_id = v_version_id
