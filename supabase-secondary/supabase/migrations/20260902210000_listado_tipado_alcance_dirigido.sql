-- Listado tipado: alcanceDirigido del borrador (para catálogo landing «para todos»).
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'alcanceDirigido', coalesce((
        select nullif(trim(d.documento->>'alcanceDirigido'), '')
        from public.documento_borrador_curso d
        where d.curso_id = c.id
      ), case
        when coalesce((
          select nullif(trim(d2.documento->>'publico'), '')
          from public.documento_borrador_curso d2
          where d2.curso_id = c.id
        ), '') ilike '%Público general%'
          then 'TODOS'
        else 'ORGANIZACION'
      end),
