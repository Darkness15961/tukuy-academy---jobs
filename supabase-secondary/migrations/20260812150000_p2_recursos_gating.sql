-- P2.5 LMS: recursos en módulo + gating secuencial.
-- Ejecutar en secundaria después de 12140000.

begin;

alter table public.modulo_curso
  add column if not exists recursos jsonb default '[]'::jsonb;

create or replace function public._servicio_sanear_recursos_modulo(p_recursos jsonb)
returns jsonb
language sql
immutable
as $$
  select coalesce(
    (
      select jsonb_agg(
        jsonb_build_object(
          'id', coalesce(nullif(trim(r->>'id'), ''), 'rec-' || ordinality::text),
          'nombre', coalesce(nullif(trim(r->>'nombre'), ''), 'Recurso'),
          'tipo', coalesce(nullif(trim(r->>'tipo'), ''), 'application/octet-stream'),
          'tamanio', coalesce((nullif(r->>'tamanio', ''))::bigint, 0),
          'contenido', case
            when coalesce(r->>'contenido', '') ~* '^(https?://|s3://)'
              then r->>'contenido'
            when left(coalesce(r->>'contenido', ''), 5) = 'data:'
              and char_length(r->>'contenido') <= 20000
              then r->>'contenido'
            else ''
          end
        )
      ...
      from jsonb_array_elements(coalesce(p_recursos, '[]'::jsonb))
        with ordinality as t(r, ordinality)
      where nullif(trim(coalesce(r->>'nombre', '')), '') is not null
    ),
    '[]'::jsonb
  );
$$;
  ...
  loop
    ...
    then
      ...
      insert into public.modulo_curso (
        id, version_curso_id, codigo, titulo, descripcion, orden, recursos
      ) values (
        v_modulo_id,
        v_version_id,
...
begin
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'title', mo.titulo,
      'orden', mo.orden,
      'recursos', coalesce(mo.recursos, '[]'::jsonb),
      'items', coalesce((
        select jsonb_agg(
          jsonb_build_object(
            'id', a.id,
...
begin
  ...
  if v_marcar then
    if exists (
      select 1
      from public.actividad_curso prev
      join public.modulo_curso mo_prev on mo_prev.id = prev.modulo_curso_id
      join public.actividad_curso cur on cur.id = p_actividad_id
      join public.modulo_curso mo_cur on mo_cur.id = cur.modulo_curso_id
      where mo_prev.version_curso_id = mo_cur.version_curso_id
        and (
          mo_prev.orden < mo_cur.orden
          or (mo_prev.orden = mo_cur.orden and prev.orden < cur.orden)
        )
        and not exists (
          select 1
          from public.progreso_actividad p
          where p.matricula_curso_id = v_matricula_id
            and p.actividad_ref = prev.id
            and upper(p.estado) in ('COMPLETADA', 'COMPLETADO', 'DONE')
        )
    ) then
      raise exception 'Debes completar las actividades anteriores primero';
    end if;
  end if;
...
revoke all on function public._servicio_sanear_recursos_modulo(jsonb)
...
grant execute on function public._servicio_sanear_recursos_modulo(jsonb) to service_role;
