-- Endurece matrícula: enums de edición/matrícula/origen/docente.
-- Ejecutar en el SQL Editor del proyecto SECUNDARIO.
-- Corrige fallos de Inscribirme cuando 20260805245000 ya existía pero
-- el insert usaba literales incompatibles con el enum real.
begin;
...
begin
  ...
  if v_edicion_id is null then
    ...
    v_estado := public._servicio_resolver_enum_tabla(
      'edicion_curso',
      'estado',
      array['ABIERTA', 'ABIERTA', 'ACTIVA', 'ACTIVO', 'EN_CURSO', 'DISPONIBLE']
    );
    ...
    if exists (
      ...
      exception
        when others then
          -- La edición ya sirve para matricular aunque falle el vínculo docente.
          null;
      end;
    end if;
  end if;
