...
const lessons = [
  ...
  {
    id: 2,
    title: "2. Herramientas y Metodología Aplicada",
    duration: "30 min",
  },
  {
    id: 3,
    title: "3. Casos de Estudio y Análisis de Errores",
    duration: "45 min",
  },
...
const assignments = [
  {
    id: 1,
    title: "Tarea 1: Registro de avance y evidencias",
    due: "Disponible",
    grade: 18,
  },
  {
    id: 2,
    title: "Tarea 2: Caso práctico aplicado a obra",
    due: "Disponible",
    grade: 17,
  },
];
...
const exams = [
  {
    id: 1,
    title: "Examen parcial: conceptos del módulo",
    questions: 12,
    grade: 16,
  },
  {
    id: 2,
    title: "Examen final: validación de competencias",
    questions: 20,
    grade: 19,
  },
];
  ...
  (newCourse) => {
    if (newCourse) {
      ...
      completedAssignments.value =
        newCourse.progress >= 60
          ? assignments.slice(0, 1).map((task) => task.id)
          : [];
      completedExams.value =
        newCourse.progress >= 90
          ? exams.slice(0, 1).map((exam) => exam.id)
          : [];
    } else {
...
const totalActivities = computed(
  () => lessons.length + assignments.length + exams.length,
);
const completedActivities = computed(
  () =>
    completedLessons.value.length +
    completedAssignments.value.length +
    completedExams.value.length,
);
...
const gradedActivities = computed(() => [
  ...assignments
    .filter((task) => completedAssignments.value.includes(task.id))
    .map((task) => task.grade),
  ...exams
    .filter((exam) => completedExams.value.includes(exam.id))
    .map((exam) => exam.grade),
]);
...
function toggleLesson(lessonId: number) {
  ...
  if (index >= 0) {
    ...
    completedLessons.value = [...completedLessons.value, ...toAdd].sort(
      (a, b) => a - b,
    );
  }
...
function toggleAssignment(taskId: number) {
  if (completedAssignments.value.includes(taskId)) {
    completedAssignments.value = completedAssignments.value.filter(
      (id) => id !== taskId,
    );
  } else {
    completedAssignments.value = [...completedAssignments.value, taskId].sort(
      (a, b) => a - b,
    );
  }
...
function toggleExam(examId: number) {
  if (completedExams.value.includes(examId)) {
    ...
    completedExams.value = [...completedExams.value, examId].sort(
      (a, b) => a - b,
    );
  }
...
<template>
  <Transition name="fade-overlay">
    ...
    >
      <!-- Top header -->
      <header
        class="flex h-16 shrink-0 items-center justify-between border-b border-slate-800 bg-slate-900/50 px-4 backdrop-blur sm:px-6"
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            <span
              class="text-[10px] font-bold uppercase tracking-wider text-blue-400 sm:text-xs"
            >
      ...
      <div class="flex flex-1 flex-col overflow-hidden lg:flex-row">
        ...
        <div class="flex flex-1 flex-col overflow-y-auto p-4 sm:p-6 lg:p-8">
          ...
          <div
            class="relative aspect-video w-full overflow-hidden rounded-xl border border-slate-800 bg-slate-900 shadow-2xl"
          >
            ...
            >
              <div
                class="rounded-full bg-blue-500/10 p-5 ring-4 ring-blue-500/20"
              >
              ...
              <p class="mt-1 max-w-md text-xs text-slate-400 sm:text-sm">
                Interactúa con el temario en la columna derecha para marcar las
                clases como completadas y simular tu avance.
              </p>
            ...
            <!-- Fake control bar -->
            <div
              class="absolute bottom-0 left-0 right-0 flex items-center justify-between bg-slate-950/80 px-4 py-2 text-xs text-slate-400"
            >
          ...
          <div class="mt-6">
            ...
            <p class="mt-2 text-sm leading-relaxed text-slate-400">
              Esta simulación interactiva te permite verificar cómo se actualiza
              la interfaz de Tukuy Academy en tiempo real. Al marcar los temas a
              la derecha, verás cómo tu progreso cambia, y al completar el 100%
              se generará automáticamente tu certificado digital verificable.
            </p>
          </div>
          ...
          <div class="mt-6 grid gap-3 sm:grid-cols-3">
            <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
              <div
                class="flex items-center gap-2 text-xs font-bold uppercase text-slate-400"
              >
                <ClipboardCheck class="h-4 w-4 text-blue-400" />
                Actividades
              </div>
              <strong class="mt-2 block text-2xl text-white"
                >{{ completedActivities }}/{{ totalActivities }}</strong
              >
            </div>
            <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
              <div
            ...
            <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
              ...
              <strong
                ...
                :class="
                  gradeStatus === 'En riesgo' ? 'text-amber-300' : 'text-white'
                "
              >
                {{ gradeStatus }}
              </strong>
        ...
        <div
          class="w-full shrink-0 border-t border-slate-800 bg-slate-900/30 backdrop-blur lg:w-96 lg:border-l lg:border-t-0 flex flex-col"
        >
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            >
              ...
              <div class="min-w-0 flex-1">
                ...
                <div
                  class="flex items-center gap-2 mt-1 text-[11px] text-slate-500"
                >
            ...
            <h3
              class="mt-5 mb-2 text-xs font-bold uppercase tracking-wider text-slate-400"
            >
              Tareas calificadas
            </h3>
