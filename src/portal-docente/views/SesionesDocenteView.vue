...
function onAsistenciaActualizada(presentes: number) {
  const sesion = sesionSeleccionada.value;
  if (!sesion) return;
  sesion.asistentes = presentes;
  const indice = sesiones.value.findIndex((item) => item.id === sesion.id);
  if (indice >= 0) sesiones.value[indice]!.asistentes = presentes;
}
        ...
        <CardContent class="p-6">
          ...
          <div class="mt-5 grid gap-3 border-y border-border py-4 text-sm">
            ...
            <span
              ...
              inscritos · {{ sesionSeleccionada.asistentes ?? 0 }}
              presentes</span
            >
          ...
          <AsistenciaSesionPanel
            v-if="asistenciaActiva"
            :sesion-id="sesionSeleccionada.id"
            @actualizado="onAsistenciaActualizada"
          />
