...
const learningCourses = computed(() => {
  const base = portal.enrolledCourses.value.length >= 8 ? portal.enrolledCourses.value : portal.courses.value
    ...
    <section class="grid gap-7">
      <div class="rounded-xl border border-[#DDE7F4] bg-[#F7F9FE] p-6 shadow-sm lg:p-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <Badge class="border-[#F5B400]/40 bg-[#F5B400]/15 text-[#0B3A78]" variant="outline">
              Mi aprendizaje
            </Badge>
            ...
            <p class="mt-3 max-w-3xl text-sm leading-7 text-[#41516A] sm:text-base">
              Revisa tu progreso, retoma cursos activos y fortalece competencias conectadas a Tukuy Obra, CV inteligente y bolsa laboral.
            </p>
          </div>

          <div class="grid gap-2 rounded-lg border border-blue-100 bg-white p-4 shadow-sm sm:min-w-64">
            <div class="flex items-center justify-between gap-3 text-sm">
              <span class="text-muted-foreground">Avance global</span>
              <strong>{{ portal.user.value?.profileProgress ?? 82 }}%</strong>
            </div>
            <Progress :model-value="portal.user.value?.profileProgress ?? 82" class="h-2.5" />
            <p class="text-xs text-muted-foreground">
              {{ portal.enrolledCourses.value.length }} cursos activos o completados
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="grid gap-5">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div class="flex flex-wrap gap-2">
            <Button class="h-10 rounded-full border-slate-200 bg-white px-4 text-slate-800" variant="outline" type="button">
              Categorías
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="relative w-full sm:w-[360px]">
              ...
              <Input v-model="searchTerm" class="h-11 bg-white pl-10" type="search" placeholder="Buscar mis cursos" />
            </div>
            <Button class="h-11 rounded-md bg-[#D6C4F2] px-4 text-[#5A35A3] hover:bg-[#CBB5EC]" type="button">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <strong class="text-sm text-slate-800">{{ learningCourses.length }} cursos</strong>
          <Button class="text-slate-800" variant="ghost" type="button">
      ...
      <Card v-if="!learningCourses.length" class="border-slate-200 shadow-none">
