-- Catálogo landing: alcance público también en configuracionPublicacion anidada.
...
begin
  ...
  from (
    ...
    where c.instalacion_proveedora_id = p_instalacion_id
      ...
      and coalesce(
        c.datos_historicos->'configuracionPublicacion'->'configuracionPublicacion'->>'alcance',
        c.datos_historicos->'configuracionPublicacion'->>'alcance',
        'TODOS'
      ) in ('PUBLICO', 'TODOS', '')
      and coalesce(
        (c.datos_historicos->'configuracionPublicacion'->'configuracionPublicacion'->>'visibleParaExternos')::boolean,
